...
-- FIXED: ESP boxes now stay on body at all distances
...
-- ════════════════════════════════════════════════════════════════════
-- ESP LOGIC — SISTEMA CORRIGIDO
-- ════════════════════════════════════════════════════════════════════

local Camera = workspace.CurrentCamera

-- Contentor de todos os drawables ESP
...
local function createEspForPlayer(player)
    ...
    rageLabel.AnchorPoint = Vector2.new(0.5, 0)
...
-- ── Loop principal do ESP (CORRIGIDO) ──
RunService.RenderStepped:Connect(function()
    ...
    for player, obj in pairs(espObjects) do
        ...
        if not anyEspOn or not hrp or not head or not hum then
            ...
            -- ── ESP BOX (CORRIGIDO) ──
            if state.toggles.ESP_BOX and visible then
                obj.box["Top"].Visible = true
                obj.box["Bottom"].Visible = true
                obj.box["Left"].Visible = true
                obj.box["Right"].Visible = true
                
                obj.box["Top"].BackgroundColor3 = espCol
                obj.box["Bottom"].BackgroundColor3 = espCol
                obj.box["Left"].BackgroundColor3 = espCol
                obj.box["Right"].BackgroundColor3 = espCol

                -- TOP (linha superior)
                obj.box["Top"].Position = UDim2.new(0, boxX, 0, boxTop)
                obj.box["Top"].Size = UDim2.new(0, boxW, 0, 2)

                -- BOTTOM (linha inferior)
                obj.box["Bottom"].Position = UDim2.new(0, boxX, 0, boxBot - 2)
                obj.box["Bottom"].Size = UDim2.new(0, boxW, 0, 2)

                -- LEFT (linha esquerda)
                obj.box["Left"].Position = UDim2.new(0, boxX, 0, boxTop)
                obj.box["Left"].Size = UDim2.new(0, 2, 0, boxH)

                -- RIGHT (linha direita)
                obj.box["Right"].Position = UDim2.new(0, boxX + boxW - 2, 0, boxTop)
                obj.box["Right"].Size = UDim2.new(0, 2, 0, boxH)
            else
                for _, line in pairs(obj.box) do line.Visible = false end
            end

            -- ── ESP NAME (CORRIGIDO) ──
            if state.toggles.ESP_NAME and visible then
                obj.name.Visible = true
                obj.name.Position = UDim2.new(0, headSP.X, 0, boxTop - 25)
                obj.name.Text = player.Name
            else
                obj.name.Visible = false
            end

            -- ── ESP RAGE (distância) (CORRIGIDO) ──
            if state.toggles.ESP_RAGE and visible then
                obj.rage.Visible = true
                local distanceText = math.floor(dist) .. "m"
                obj.rage.Text = distanceText
                obj.rage.Position = UDim2.new(0, headSP.X, 0, boxTop - 45)
            else
                obj.rage.Visible = false
            end

            -- ── ESP HEAD ──
            if state.toggles.ESP_HEAD and visible then
                obj.head.Visible = true
                local headSize = math.max(10, boxW * 0.6)
                obj.head.Position = UDim2.new(0, headSP.X, 0, topSP.Y)
                obj.head.Size = UDim2.new(0, headSize, 0, headSize)
                obj.head.BackgroundColor3 = espCol
            else
                obj.head.Visible = false
            end

            -- ── ESP TRACE ──
            if state.toggles.ESP_TRACE and visible then
                obj.trace.Visible = true
                obj.trace.BackgroundColor3 = espCol
                local screenCenterY = 0
                obj.trace.Position = UDim2.new(0, headSP.X - 1, 0, screenCenterY)
                obj.trace.Size = UDim2.new(0, 2, 0, topSP.Y - screenCenterY)
            else
                obj.trace.Visible = false
            end

            -- ── ESP LIFE ──
            if state.toggles.ESP_LIFE and visible and hum then
                local healthPercent = hum.Health / hum.MaxHealth
                obj.lifeBG.Visible = true
                obj.lifeBG.Position = UDim2.new(0, boxX + boxW + 5, 0, boxTop)
                obj.lifeBG.Size = UDim2.new(0, 4, 0, boxH)
                obj.lifeBar.Size = UDim2.new(1, 0, healthPercent, 0)
                obj.lifeBar.BackgroundColor3 = Color3.fromRGB(50 + (1 - healthPercent) * 205, 220 * healthPercent, 50)
            else
...
print("[ShadowMenu] ✅ ESP Sistema Corrigido - Loaded. Press INSERT to toggle.")
